...
window.enhanceMarkdownContent = (container) => {
  ...
  root.querySelectorAll('img').forEach((img) => {
    // Skip small UI icons, avatar pictures, preview mockup, logos, footer/header, and LCP/hero images
    if (img.getAttribute('fetchpriority') === 'high' || img.getAttribute('loading') === 'eager' || img.classList.contains('blog-hero-image') || img.closest('a') || img.closest('header') || img.closest('footer') || img.classList.contains('w-8') || img.classList.contains('w-10') || img.classList.contains('w-12') || img.id === 'resource-preview-img' || img.closest('.site-navigation') || img.closest('.unlock-card-container')) return;
